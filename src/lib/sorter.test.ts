...
const countItems = (s: SorterState) =>
  s.done!.reduce((n, group) => n + group.length, 0);
...
describe('ソート結果', () => {
  ...
  it.each([2, 3, 7, 16, 33, 62])(
    '%i 件をランダムな順序で正しく並べ替える',
    n => {
...
describe('maxComparisons', () => {
  ...
  it.each([2, 3, 7, 16, 33, 62])('%i 件の実際の比較回数が上限を超えない', n => {
    ...
    for (let trial = 0; trial < 5; trial++) {
      ...
      expect(autoSort(truth, false).comparisons).toBeLessThanOrEqual(
        maxComparisons(n),
      );
    }
  });
