...
describe('filterRows', () => {
  ...
  it('上位 N 件に切り詰める', () => {
    expect(filterRows(rows, items, new Set(), 2).map(r => r.index)).toEqual([
      0, 1,
    ]);
  });
...
describe('isRankHead', () => {
  ...
  it('先頭と、順位が変わる行では順位を表示する', () => {
    expect(rows.map((_, i) => isRankHead(rows, i))).toEqual([
      true,
      true,
      false,
      true,
  ...
  it('絞り込みで同順位の 1 件目が消えた場合は、残った先頭に順位を表示する', () => {
    ...
    expect(filtered.map((_, i) => isRankHead(filtered, i))).toEqual([
      true,
      true,
    ]);
  });
