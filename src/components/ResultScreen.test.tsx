...
const result: Run = [[0], [1, 2], [3], ...Array.from({ length: 8 }, (_, i) => [i + 4])]
...
function visibleLabels(): string[] {
  return items.map((i) => i.label).filter((label) => screen.queryByText(label) !== null)
}

/** グループ絞り込みの「すべて」チップ (表示件数にも同名のボタンがあるため区別する) */
const allGroupsChip = () =>
  screen.getAllByText('すべて').find((el) => el.className.includes('MuiChip-label'))!
...
describe('ResultScreen の画像保存', () => {
  ...
  const saveButton = () => screen.getByText('画像で保存').closest('button') as HTMLButtonElement
  ...
  it('生成に失敗したらエラーを表示する', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/画像の保存に失敗しました/)).toBeInTheDocument())
