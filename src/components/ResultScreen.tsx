...
export default function ResultScreen(props: Props) {
  ...
  const groups = createMemo(() =>
    availableGroups(props.items, props.groupOrder),
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(
        '画像の保存に失敗しました。この画面を表示したままもう一度お試しください。',
      );
                  ...
                  >
                    <Box
                      sx={{
                        ...
                        color:
                          row.rank <= 3 ? 'primary.main' : 'text.secondary',
                      }}
                    >
                      {showRank() ? row.rank : '='}
                    </Box>
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography sx={{ fontWeight: 700, ...ELLIPSIS }}>
                        {item().label}
                      </Typography>
                      <Typography
                        variant="caption"
